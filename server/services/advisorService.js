...
export async function advisorChat(userId, query) {
  ...
  const prompt = `
    ...
    Use this context JSON: ${JSON.stringify(context)}
    User query: ${query}
    Respond with strict JSON: {"reply":"short practical advice"}
  `.trim();

  // Try mode-specific provider first
  if (env.aiMode === "groq" && env.groqApiKey) {
    try {
      ...
      if (content?.reply) return { reply: content.reply, context };
    } catch (err) {
      console.error("[AdvisorService] Groq Primary Error:", err.message);
    }
  }
  ...
  if (env.geminiApiKey) {
    try {
      ...
      if (model?.reply) return { reply: model.reply, context };
    } catch (error) {
      console.error("[AdvisorService] Gemini Error:", error.message);
      // Final fallback to Groq text if Gemini failed but we had a key
      if (env.groqApiKey && env.aiMode !== "groq") {
        try {
          const groqResponse = await axios.post("https://api.groq.com/openai/v1/chat/completions", {
            model: "llama-3.3-70b-versatile",
            messages: [{ role: "user", content: prompt }],
